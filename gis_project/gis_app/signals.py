...
from .tasks import update_avg_cords_task, update_user_vehicles_names_m2m_task, update_user_vehicles_task

@receiver(post_save,
          sender="gis_app.UserPosition",
          dispatch_uid="update_avg_coords")
def update_avg_cords(sender, instance, **kwargs):
    update_avg_cords_task.delay(instance.id)
...
def update_user_vehicles_names_m2m(sender, instance, action, **kwargs):
    ...
    if action in ["post_add", "post_remove"]:
        update_user_vehicles_names_m2m_task.delay(list(user_pk_set))
...
def update_user_vehicles(sender, instance, **kwargs):
    update_user_vehicles_task.delay(instance.id)
